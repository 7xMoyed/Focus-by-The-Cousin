...
async function publishToVenueBranches(supabase: AnyClient, candidateId: string) {
    ...
    .upsert(
      {
        ...
        name_en: c.name_en,
        slug: venueSlug,
        venue_type: c.venue_type || "cafe",
        description_ar: c.why_it_may_fit ?? null,
        ...(c.official_instagram_url
          ? {
              instagram_handle: c.official_instagram_url
  ...
  if (existing) {
    ...
    const { error: insertErr } = await supabase.from("venue_branches").insert(branchData);
